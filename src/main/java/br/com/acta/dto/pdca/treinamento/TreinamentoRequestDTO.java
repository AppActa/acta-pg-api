...
public record TreinamentoRequestDTO(
        @Schema(description = "Título do treinamento", example = SwaggerRequestExamples.TITULO_TREINAMENTO)
        @NotBlank(message = "{validation.titulo.notblank}")
        @Size(max = 160, message = "{validation.titulo.size}")
        String titulo,

        @Schema(description = "Descrição do treinamento", example = SwaggerRequestExamples.DESCRICAO_TREINAMENTO)
        ...
        @Schema(description = "Obrigatório", example = SwaggerRequestExamples.OBRIGATORIO)
