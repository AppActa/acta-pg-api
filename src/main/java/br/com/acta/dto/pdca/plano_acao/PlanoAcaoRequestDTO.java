...
public record PlanoAcaoRequestDTO(
        @Schema(description = "Nome do plano de ação", example = SwaggerRequestExamples.NOME_PLANO_ACAO)
        @NotBlank(message = "{validation.planoAcao.nome.notblank}")
        @Size(max = 160, message = "{validation.planoAcao.nome.size}")
        String nome,

        @Schema(description = "Objetivo do plano de ação", example = SwaggerRequestExamples.OBJETIVO_PLANO_ACAO)
        @Size(max = 1000, message = "{validation.objetivo.size}")
        String objetivo,

        @Schema(description = "Prioridade do plano de ação", implementation = Prioridade.class, example = SwaggerRequestExamples.PRIORIDADE)
        @NotNull(message = "{validation.prioridade.notnull}")
        Prioridade prioridade,

        @Schema(description = "Origem do plano de ação", implementation = OrigemRegistro.class, example = SwaggerRequestExamples.ORIGEM)
