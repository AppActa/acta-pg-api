...
public record MetaRequestDTO(
        @Schema(description = "Objetivo da meta", example = SwaggerRequestExamples.OBJETIVO_META)
        @NotBlank(message = "{validation.objetivo.notblank}")
        @Size(max = 1000, message = "{validation.objetivo.size}")
        String objetivo,

        @Schema(description = "Valor base da meta", example = SwaggerRequestExamples.VALOR_BASE)
        ...
        @Schema(description = "Valor alvo da meta", example = SwaggerRequestExamples.VALOR_ALVO)
        ...
        @Schema(description = "Unidade de medida da meta", example = SwaggerRequestExamples.UNIDADE_MEDIDA)
        ...
        @Schema(description = "Prioridade da meta", implementation = Prioridade.class, example = SwaggerRequestExamples.PRIORIDADE)
        @NotNull(message = "{validation.prioridade.notnull}")
        Prioridade prioridade,

        @Schema(description = "Área da meta", example = SwaggerRequestExamples.AREA)
        @Size(max = 100, message = "{validation.area.size}")
        String area,

        @Schema(description = "Categoria da meta", example = SwaggerRequestExamples.CATEGORIA)
        @Size(max = 100, message = "{validation.meta.categoria.size}")
        String categoria,

        @ArraySchema(schema = @Schema(implementation = Long.class, example = SwaggerRequestExamples.ID_RESPONSAVEL), minItems = 1, uniqueItems = true, arraySchema = @Schema(description = "Lista de IDs de responsáveis pela meta"))
