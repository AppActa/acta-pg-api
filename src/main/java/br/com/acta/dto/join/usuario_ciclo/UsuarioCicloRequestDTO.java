...
public record UsuarioCicloRequestDTO(
        @Schema(description = "ID do usuario", example = SwaggerRequestExamples.ID_USUARIO)
        @NotNull(message = "{validation.idUsuario.notnull}")
        @Positive(message = "{validation.idUsuario.positive}")
        Long idUsuario,

        @Schema(description = "Papel do colaborador no ciclo", example = SwaggerRequestExamples.PAPEL_CICLO, implementation = PapelCiclo.class)
