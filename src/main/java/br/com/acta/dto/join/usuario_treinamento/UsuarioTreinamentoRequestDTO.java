...
public record UsuarioTreinamentoRequestDTO(
        @Schema(description = "ID do colaborador", example = SwaggerRequestExamples.ID_USUARIO)
        @NotNull(message = "{validation.idUsuario.notnull}")
        @Positive(message = "{validation.idUsuario.positive}")
        Long idUsuario,

        @Schema(description = "Indica se o colaborador é obrigatório no treinamento", example = SwaggerRequestExamples.PRINCIPAL)
