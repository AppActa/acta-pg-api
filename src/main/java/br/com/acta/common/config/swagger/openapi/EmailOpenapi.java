...
public interface EmailOpenapi {
    ...
    @ApiBadRequestResponse
    @ApiNotFoundResponse
    ResponseEntity<List<EmailResponseDTO>> buscarEmailColaborador(@Parameter(description = SwaggerParameterDescriptions.ID_COLABORADOR) Long idColaborador);

    @Operation(summary = "Adiciona um e-mail a um colaborador")
    @ApiResponse(responseCode = "201", description = "E-mail adicionado", content = @Content(schema = @Schema(implementation = EmailResponseDTO.class)))
    @ApiResourceResponses
    @ApiUnsupportedMediaTypeResponse
    ResponseEntity<EmailResponseDTO> inserirEmailColaborador(@Parameter(description = SwaggerParameterDescriptions.ID_COLABORADOR) Long idColaborador, @RequestBody(description = "Dados do e-mail", required = true) EmailRequestDTO dto);

    @Operation(summary = "Exclui um e-mail de um colaborador")
    @ApiResponse(responseCode = "204", description = "E-mail excluído")
    @ApiResourceResponses
    ResponseEntity<Void> excluirEmailColaborador(@Parameter(description = SwaggerParameterDescriptions.ID_COLABORADOR) Long idColaborador, @Parameter(description = SwaggerParameterDescriptions.ID_EMAIL) Long idEmail);
}
